...
class Components
{
  public static function h2($text) {
    return <<<EOM
      <h2 class="display-5 mb-3">{$text}</h2>
    EOM;
  }

  public static function h3($text) {
    return <<<EOM
      <h4 class="mb-2">{$text}</h3>
    EOM;
  }

  public static function h4($text) {
    return <<<EOM
      <h5 class="mb-1">{$text}</h4>
    EOM;
  }
  ...
  public static function infoBtn() {
    return <<<EOM
      <a href="/support" class="text-secondary"><i class="fa-solid fa-circle-info"></i></a>
    EOM;
  }

  public static function watchOnAdmin($children) {
    return <<<EOM
      <div class="toast-container position-fixed top-0 start-50 p-3 translate-middle-x">
        <div class="p-2 align-items-center text-bg-warning border-0" role="alert" aria-live="assertive" aria-atomic="true">
          <div class="d-flex">
            <div class="toast-body">{$children}</div>
          </div>
        </div>
      </div>
    EOM;
